...
import { MapContainer, TileLayer, Polygon, Polyline, Marker, CircleMarker, Circle, Popup, useMap, useMapEvent } from "react-leaflet";
import L from "leaflet";
import "leaflet/dist/leaflet.css";
import { NEWRY_VIEW, BASEMAPS, latlngFor, REGIONAL_VIEW, REGIONAL_FORECAST, NEWRY_CATCHMENT, FLOOD_EXTENT, STREET_RISK, TOWN_VIEW } from "./src/map/geoNewry.js";
import { FigIcon, FloodIqMark, FloodAvatar, AlertPulse, IMG } from "./src/ui/FigIcon.jsx";
import { fetchNiFloods } from "./src/map/niFloods.js";
import { fetchEaPolygon, geoJsonToLatLngs } from "./src/map/eaFloods.js";
...
const GROUP_META = {
  reservoir: { label: "Canal pound",      ground: "above", short: "RES" },
  ...
  drainage:  { label: "Town drainage",    ground: "above", short: "DRN" },
  ground:    { label: "Groundwater",      ground: "below", short: "GW"  },
  met:       { label: "Met station",      ground: "above", short: "MET" },
};

/* ==================================================================
   ASSETS — Newry Flood Alleviation Scheme, Newry River corridor
   through Newry city centre. Coordinates are a PLACEHOLDER; swap for
   survey-grade GIS data once available.
   ================================================================== */
const seedAssets = [
  { id: "res-01", name: "Canal pound level", ref: "STR-RES-01", group: "reservoir", unit: "mAOD",
  ...
  { id: "res-fb", name: "Crest freeboard", ref: "STR-RES-FB", group: "reservoir", unit: "m", invert: true,
    ...
    note: "Derived: flood wall crest minus water level. Falls as the canal pound fills." },
  { id: "spw-01", name: "Spillway weir", ref: "STR-SPW-01", group: "structure", unit: "m³/s",
    ...
    note: "Ogee overflow to the Newry River. Dry below top pound level." },
  { id: "dot-01", name: "Draw-off tower", ref: "STR-DOT-01", group: "structure", unit: "% open",
    ...
    note: "Controlled release to the Newry River. Operator setpoint." },
  { id: "pz-04", name: "Embankment piezometers P-04", ref: "STR-PZ-04", group: "embankment", unit: "kPa",
    ...
    note: "Pore pressure, SW flood wall. Lags canal pound level by ~6h." },
  ...
  { id: "riv-col", name: "Newry River at Victoria Lock", ref: "NI-NRV-14", group: "river", unit: "m",
    level: 1.20, min: -2.5, max: 3.4, alert: 2.10, warning: 2.75, severe: 3.09, x: 168, y: 400, resp: 1.2,
    note: "Tidal gauge — oscillates with Carlingford Lough twice daily. Peaked 3.09m AOD (Belfast) on 30–31 Oct 2023; DfI Rivers Review of South East Flooding, Autumn 2023, Fig 3-7." },
  { id: "riv-lon", name: "Camlough River confluence", ref: "NI-CAM-03", group: "river", unit: "m",
    level: 1.21, min: 0, max: 2.4, alert: 1.15, warning: 1.55, severe: 1.95, x: 252, y: 352, resp: 0.95,
    note: "Joins the Newry north of the town centre. Culverted under the Camlough Road." },
  { id: "drn-e", name: "Edward Street balancing pond", ref: "NMD-BP-E", group: "drainage", unit: "% full",
    level: 66, min: 0, max: 100, alert: 62, warning: 80, severe: 93, x: 688, y: 262, resp: 1.35,
    note: "Serves the Edward Street catchment. Discharge consent limits release rate." },
  { id: "drn-n", name: "Northern balancing pond", ref: "NMD-BP-N", group: "drainage", unit: "% full",
    level: 58, min: 0, max: 100, alert: 62, warning: 80, severe: 93, x: 448, y: 156, resp: 1.25,
    note: "Serves the Newry Canal towpath and northern approaches." },
  { id: "gw-st", name: "Bessbrook Road gravels BH-11", ref: "NI-GW-11", group: "ground", unit: "mAOD",
  ...
  { id: "met-01", name: "Town centre rain gauge", ref: "NMD-MET-01", group: "met", unit: "mm/h",
    level: 16.40, min: 0, max: 35, alert: 8, warning: 15, severe: 24, x: 560, y: 214, resp: 1.0,
    note: "Tipping bucket, 1-min resolution." },
  { id: "met-02", name: "Sugar Island rain gauge", ref: "NMD-MET-02", group: "met", unit: "mm/h",
    level: 14.20, min: 0, max: 35, alert: 8, warning: 15, severe: 24, x: 710, y: 230, resp: 1.0,
    note: "Tipping bucket on the Sugar Island flood wall. Feeds the town drainage model." },
  { id: "met-03", name: "Bessbrook rain gauge", ref: "NMD-MET-03", group: "met", unit: "mm/h",
    level: 13.80, min: 0, max: 35, alert: 8, warning: 15, severe: 24, x: 340, y: 190, resp: 1.0,
    note: "Tipping bucket at Bessbrook. Upstream catchment indicator." },
  { id: "met-04", name: "Camlough gauge", ref: "NMD-MET-04", group: "met", unit: "mm/h",
    level: 18.10, min: 0, max: 35, alert: 8, warning: 15, severe: 24, x: 540, y: 420, resp: 1.0,
    note: "Village gauge at Camlough. Validates upland catchment rainfall." },
  { id: "met-05", name: "Newtownhamilton road gauge", ref: "NMD-MET-05", group: "met", unit: "mm/h",
    level: 15.60, min: 0, max: 35, alert: 8, warning: 15, severe: 24, x: 380, y: 310, resp: 1.0,
    note: "Gauge on the Newtownhamilton road. Monitors upstream Newry headwater input." },
  { id: "met-06", name: "Merchants Quay flood-wall gauge", ref: "NMD-MET-06", group: "met", unit: "mm/h",
    level: 12.90, min: 0, max: 35, alert: 8, warning: 15, severe: 24, x: 490, y: 230, resp: 1.0,
    note: "Flush-mounted gauge on the Merchants Quay flood wall. Critical for standing water alerts." },
  { id: "riv-ct", name: "Newry River at Newry Bus Station", ref: "NI-NRV-15", group: "river", unit: "m",
    level: 2.60, min: 0, max: 5.0, alert: 2.75, warning: 3.00, severe: 3.30, x: 200, y: 360, resp: 1.15,
    note: "Peaked 3.30m AOD (Belfast) on 30–31 Oct 2023 — approx. 1.3m below the flood level on Sugar Island. Real gauge; DfI Rivers Review of South East Flooding, Autumn 2023, Fig 3-7." },
  { id: "riv-cb", name: "Camlough River at Bessbrook", ref: "NI-CAM-02", group: "river", unit: "m",
    level: 1.87, min: 0, max: 3.0, alert: 1.60, warning: 2.10, severe: 2.50, x: 350, y: 280, resp: 1.1,
    note: "Tributary joining the Newry at Bessbrook. Flashy response, 1–2h lag to peak." },
  { id: "riv-tw", name: "Newry River at Carlingford Lough", ref: "NI-NRT-08", group: "river", unit: "m",
  ...
  { id: "drn-w", name: "Western balancing pond", ref: "NMD-BP-W", group: "drainage", unit: "% full",
    level: 42, min: 0, max: 100, alert: 62, warning: 80, severe: 93, x: 310, y: 220, resp: 1.3,
    note: "Serves Bessbrook Road and western catchments. Discharge to the Newry Canal." },
  { id: "drn-s", name: "Southern detention basin", ref: "NMD-BP-S", group: "drainage", unit: "% full",
    level: 55, min: 0, max: 100, alert: 62, warning: 80, severe: 93, x: 420, y: 380, resp: 1.4,
    note: "Attenuates runoff from southern catchments before release to the Newry." },
  { id: "drn-rw", name: "Merchants Quay culvert catchpit", ref: "NMD-CP-01", group: "drainage", unit: "% full",
    level: 71, min: 0, max: 100, alert: 65, warning: 85, severe: 95, x: 530, y: 250, resp: 1.5,
    note: "Collects Merchants Quay culvert runoff. High level triggers town-centre road closure protocol." },
  { id: "gw-sm", name: "Camlough BH-07", ref: "NI-GW-07", group: "ground", unit: "mAOD",
  ...
  { id: "gw-lh", name: "Bessbrook Road BH-03", ref: "NI-GW-03", group: "ground", unit: "mAOD",
    level: 16.80, min: 12, max: 22, alert: 17.5, warning: 18.8, severe: 20.0, x: 270, y: 340, resp: 0.3,
    note: "Monitoring well near Bessbrook Road. Lags rainfall by 24–48h." },
  { id: "str-bw", name: "Camlough upland spring", ref: "NI-GW-12", group: "ground", unit: "l/s",
    level: 2.10, min: 0, max: 8, alert: 3.0, warning: 5.0, severe: 6.5, x: 180, y: 300, resp: 0.5,
    note: "Upland spring feeding the Newry. Discharge rises 12–24h after recharge events." },
];

const seedFeeds = [
  { id: 1, source: "official", who: "DfI Rivers Floodline", when: "3 min", text: "Flood Alert in force — Newry River including Camlough, Bessbrook and Newry city centre." },
  { id: 2, source: "official", who: "Met Office", when: "11 min", text: "Amber warning for rain, County Down and County Armagh, 16:00 today to 09:00 tomorrow." },
  { id: 3, source: "ops", who: "NMD Council Ops", when: "16 min", text: "Merchants Quay car park standing water reported by street cleansing crew. Pump deployed." },
  { id: 4, source: "sensor", who: "Telemetry", when: "22 min", text: "P-04 piezometer array rising 3.1 kPa/h — above modelled response for current canal pound level." },
  { id: 5, source: "media", who: "Newry Times", when: "34 min", text: "Newry residents told to prepare as Newry levels climb for third day running." },
  { id: 6, source: "social", who: "@camloughflood", when: "41 min", text: "Water over the road at the Sugar Island junction again. Same spot as 2017." },
  { id: 7, source: "ops", who: "DfI Rivers Duty Engineer", when: "52 min", text: "Requesting forecast for Merchants Quay culvert crossing before evening high tide." },
  { id: 8, source: "official", who: "NIFRS", when: "1 hr", text: "Pre-positioned rescue units at Newry fire station. Water rescue teams on standby." },
  { id: 9, source: "sensor", who: "Telemetry", when: "1 hr", text: "Canal inflow rate exceeded outflow by 12 m³/s in last hour. Level rising." },
  { id: 10, source: "social", who: "@NewryToday", when: "1 hr", text: "Hill Street pavement flooding near the Town Hall. Pedestrians being diverted." },
  { id: 11, source: "ops", who: "Street Cleansing", when: "2 hr", text: "Van stuck on flooded Canal Street. Recovery vehicle dispatched." },
  { id: 12, source: "official", who: "DfI Rivers Floodline", when: "2 hr", text: "Newry River at Newry Bridge 0.3m below warning level. Rate of rise slowing." },
  { id: 13, source: "media", who: "BBC News NI", when: "2 hr", text: "Council monitoring situation closely. No road closures reported so far." },
  { id: 14, source: "sensor", who: "Telemetry", when: "3 hr", text: "Camlough River at Bessbrook approaching alert threshold. Gauge reading 1.87m." },
  { id: 15, source: "social", who: "@NewryFloodWatch", when: "3 hr", text: "Rain gauge at Merchants Quay shows 28mm in last 3 hours. Umbrellas getting a workout in town." },
];

const FEED_META = {
  official: { label: "Official", color: T.blue800 },
  ops:      { label: "Council ops", color: "#0F7C6B" },
...
const AUDIENCES = [
  { id: "public", label: "Downstream residents", detail: "Newry · Camlough · Bessbrook · Warrenpoint", est: "4,800 opted-in" },
  { id: "ops",    label: "Council operations",   detail: "NMD Council ops · DfI Rivers duty · street cleansing", est: "85 staff" },
  { id: "emerg",  label: "Emergency services",   detail: "PSNI · NIFRS · DfI Rivers duty engineer",     est: "2 control rooms" },
];

const NI_WARNINGS = [
  { id: "NI-NRV-01", severity: "severe", label: "Severe Flood Warning", area: "Newry River through Newry city centre", river: "Newry River", raised: "14:32 today", message: "Flooding is expected. Act now. River levels are rising rapidly and flood water may affect properties in Newry city centre." },
  { id: "NI-CAM-02", severity: "warning", label: "Flood Warning", area: "Camlough River at Bessbrook confluence", river: "Camlough River", raised: "13:18 today", message: "Flooding is possible. Monitor closely. Camlough River levels are rising and may affect low-lying properties near Bessbrook." },
  { id: "NI-CAN-03", severity: "warning", label: "Flood Warning", area: "Newry Canal, Sugar Island to Victoria Lock", river: "Newry Canal", raised: "12:45 today", message: "Flooding is possible. Be prepared. Canal levels remain high and may affect towpaths and low-lying roads." },
  { id: "NI-NRV-04", severity: "alert", label: "Flood Alert", area: "Upper Newry River near Camlough Road", river: "Newry River", raised: "11:02 today", message: "Flood alert in force. River levels are being monitored. Surface water flooding possible in low-lying areas." },
  { id: "NI-NRT-05", severity: "alert", label: "Flood Alert", area: "Newry River tidal reach at Carlingford Lough", river: "Newry River", raised: "10:30 today", message: "Flood alert in force. Monitor river levels. Property flooding not currently expected but remain vigilant." },
  { id: "NI-CAM-06", severity: "alert", label: "Flood Alert", area: "Camlough River near Camlough village", river: "Camlough River", raised: "09:15 today", message: "Flood alert in force. River levels are elevated. Localised surface water flooding possible during heavy rain." },
];

// Real timeline of the Oct–Nov 2023 South East NI flood event affecting Newry.
// Source: DfI Rivers, "Review of Flooding in the South East, Autumn 2023" (Aug 2024),
// Section 2.2 "Timeline" (Figure 2-2) and Table 3-13.
const FLOOD_2023_TIMELINE = [
  { date: "18–20 Oct", time: "", title: "Storm Babet rainfall", detail: "Storm Babet brings heavy rainfall across the South East, saturating catchments ahead of the main event.", tag: "weather" },
  { date: "23 Oct", time: "", title: "Lough Neagh sluices opened", detail: "DfI Rivers opens the Toome sluice gates on Lough Neagh as a precautionary measure.", tag: "action" },
  { date: "26 Oct", time: "", title: "Rainfall event begins", detail: "A prolonged, intense rainfall event begins across the South East catchments, including the Newry River.", tag: "weather" },
  { date: "27 Oct", time: "", title: "Multi-agency responders informed", detail: "DfI Rivers formally informs multi-agency responders (PSNI, NIFRS, councils) that a significant flood risk is developing.", tag: "action" },
  { date: "28 Oct", time: "", title: "Multi-Agency Co-ordination begins", detail: "Multi-agency co-ordination arrangements stand up ahead of the forecast peak.", tag: "action" },
  { date: "30 Oct", time: "09:00", title: "Met Office Amber Warning issued", detail: "Amber rainfall warning in force 30 Oct 09:00 to 31 Oct 21:00 for the South East, including Newry.", tag: "weather", severity: "warning" },
  { date: "30 Oct", time: "15:00", title: "Victoria Lock sluices opened", detail: "Newry, Mourne and Down District Council opens the Victoria Lock sluices; tidal cycling of the lock continues through the event.", tag: "action" },
  { date: "30 Oct", time: "evening", title: "DfI MERP activated", detail: "DfI's Major Emergency Response Procedure is activated; DfI's Departmental Emergency Management Group meets.", tag: "action" },
  { date: "30 Oct", time: "20:00", title: "PSNI Gold Command established", detail: "PSNI stands up Gold Command to coordinate the multi-agency response.", tag: "action" },
  { date: "31 Oct", time: "03:00–05:00", title: "Wall collapse near Town Hall (FD1161)", detail: "A masonry wall on the right bank of the Newry River, ~20m downstream of Newry Town Hall, collapses into the river under floodwater load after a timber hoarding was removed. No additional flooding caused; may have helped drain water, but leaves a low spot for future events.", tag: "incident", severity: "severe" },
  { date: "31 Oct", time: "07:00", title: "Newry Hockey Club gauge peaks", detail: "River level gauge near Newry Hockey Club records its peak for the event.", tag: "gauge" },
  { date: "31 Oct", time: "", title: "Flood bank overtopped (FD164)", detail: "The flood bank north of the city, between the river and canal, is overtopped by high Newry River levels — no structural failure, but capacity is exceeded, causing significant flooding in Newry city centre (Sugar Island, New Street, Canal Quay).", tag: "incident", severity: "severe" },
  { date: "31 Oct", time: "", title: "DfI Strategic Co-ordination Group established", detail: "DfI stands up a Strategic Co-ordination Group, taking over from the PSNI-led group.", tag: "action" },
  { date: "1 Nov", time: "", title: "Rainfall event ends", detail: "The prolonged rainfall event across the South East comes to an end.", tag: "weather" },
  { date: "2 Nov", time: "19:30", title: "Victoria Lock sluices closed", detail: "NMD Council closes the Victoria Lock sluices as river and tidal levels recede.", tag: "action" },
  { date: "21 Nov", time: "", title: "Recovery stood down", detail: "The Recovery Co-ordination Group formally stands down.", tag: "action" },
  { date: "10 Dec", time: "", title: "Lough Neagh sluices still open", detail: "Toome sluice gates remain open, reflecting the scale and duration of the wider catchment response.", tag: "note" },
];

// Two real historical incidents from the same event, for the "Load 2023 event" action.
const FLOOD_2023_INCIDENTS = [
  {
    ref: "FD164", title: "Flood bank overtopped — north of city centre", severity: "severe",
    assetIds: ["riv-ct"],
    opened: "31 Oct 2023",
    log: [
      { t: "31 Oct 2023", text: "Flood bank between the Newry River and the Newry Canal, north of the city, overtopped by high river levels. No structural failure — defence exceeded its capacity." },
      { t: "31 Oct 2023", text: "Significant flooding reported in Newry city centre: Sugar Island, New Street, Canal Quay, Sugar House Quay, Merchants Quay, Basin Walk." },
    ],
  },
  {
    ref: "FD1161", title: "Masonry wall collapse — near Town Hall", severity: "warning",
    assetIds: ["riv-col"],
    opened: "31 Oct 2023, 03:00–05:00",
    log: [
      { t: "31 Oct 2023 03:00", text: "Masonry wall on the right bank of the Newry River, approx. 20m downstream of Newry Town Hall, collapsed into the river." },
      { t: "31 Oct 2023 05:00", text: "Mechanism: floodwater load on the wall after a timber hoarding was removed. No additional flooding attributed to the collapse; may have assisted drainage. Leaves a low-spot risk for future events." },
    ],
  },
];

/* Duty roster mock data — supports the client's "Internal: assets, people,
   shifts, assign teams" ask. Illustrative shift pattern, not real staff. */
const DUTY_TEAMS = [
  { id: "team-a", name: "Team A — River & Defences", shift: "On shift · 07:00–19:00", members: ["D. McParland (lead)", "S. Quinn"], org: "DfI Rivers" },
  { id: "team-b", name: "Team B — Pumping & Drainage", shift: "On shift · 07:00–19:00", members: ["R. Fleming", "C. Hughes"], org: "NI Water" },
  { id: "team-c", name: "Team C — Roads & Sandbags", shift: "Standby · callout", members: ["P. Marmion", "A. Cunningham"], org: "NMD Council" },
  { id: "team-d", name: "Night cover", shift: "On shift · 19:00–07:00", members: ["J. Trainor"], org: "DfI Rivers" },
];
...
/* Scenario projection — canal/flood-wall-centric.
   Pre-emptive drawdown buys freeboard but loads the Newry. */
...
/* ==================================================================
   Map — OpenStreetMap basemap over Newry + the flood alleviation scheme.
...
/* NI flood-warning marks. Same house-and-waves flood icon used across the UK,
   reused here since it's a generic flood-risk symbol (no Environment Agency
   text/branding baked in) — recoloured per severity, not attributed to the EA. */
const NI_FLOOD_ICON = { alert: IMG.eaAlert, warning: IMG.eaWarning, severe: IMG.eaSevere };
function niMarkHtml(tier) {
  const src = NI_FLOOD_ICON[tier];
  if (!src) return "";
  return (
...
/* Small inline version for React (non-Leaflet) contexts, such as
   the Legend and the warning detail panel — same icon as niMarkHtml above. */
function NiMark({ tier, size = 15 }) {
  const src = NI_FLOOD_ICON[tier];
  if (!src) return null;
  return <img src={src} alt="" style={{ width: size, height: size * 0.93, flexShrink: 0 }} />;
}

/* NI flood warnings, as a Leaflet layer. Boundaries would be fetched on
   selection if a warning carries a `.polygon` URL — demo items don't, so
   this is a no-op today, kept ready for a real DfI Rivers integration. */
    ...
    <>
      ...
      {data.items.map((f) => (
        <Marker key={f.id} position={[f.lat, f.long]}
          icon={L.divIcon({ html: niMarkHtml(f.tier), className: "fiq-marker", iconSize: [0, 0] })}
...
/* FakeRainOverlay — dummy rain bands that animate across the Newry area
...
function FakeRainOverlay({ show }) {
  ...
  /* Generate rain cell shapes — irregular polygons around Newry */
  const cells = useMemo(() => {
    const base = [
...
function MoveTools() {
  ...
  const tools = [
    { id: "north", icon: "northStar", title: "Reset view",
      onClick: () => { map.setView(NEWRY_VIEW.center, NEWRY_VIEW.zoom, { duration: 0.4 }); } },
...
/* Flies the map to the right framing for the current "detail level" —
   Regional (zoomed out, forecast blob), Level 2 (town-centre flood
   extent) or Level 3 (street-level) all use the tighter town view;
   Level 1 (default telemetry) returns to the scheme-wide framing. */
function DetailLevelFlyer({ detailLevel }) {
  const map = useMap();
  useEffect(() => {
    if (detailLevel === "regional") {
      map.flyTo(REGIONAL_VIEW.center, REGIONAL_VIEW.zoom, { duration: 0.6 });
    } else if (detailLevel === "l2" || detailLevel === "l3") {
      map.flyTo(TOWN_VIEW.center, TOWN_VIEW.zoom, { duration: 0.6 });
    } else {
      map.flyTo(NEWRY_VIEW.center, NEWRY_VIEW.zoom, { duration: 0.6 });
    }
  }, [detailLevel]);
  return null;
}

const STREET_RISK_COLOR = { high: T.red800, medium: T.orange600, low: T.blueP2 };

function NewryMap({ assets, mode, params, selectedId, onSelect,
                      eaFloods, eaSelected, setEaSelected, showEa, histories,
                      activeTool, subTool, cursorPos, setCursorPos,
                      panToId, setPanToId, detailLevel }) {
  ...
  return (
    <div className="absolute inset-0" style={{ background: T.land, zIndex: 0, cursor: getCursorStyle() }}>
      <MapContainer center={NEWRY_VIEW.center} zoom={NEWRY_VIEW.zoom}
        minZoom={NEWRY_VIEW.minZoom} maxZoom={NEWRY_VIEW.maxZoom}
        ...
        <DetailLevelFlyer detailLevel={detailLevel} />
        <GroupSelectOverlay active={activeTool === "rectangleSelect"} subTool={subTool}
          assets={assets} onSelectIds={(ids) => setSelectedIds(ids)} />

        {/* Newry Canal / River are rendered accurately by the OSM basemap
            itself — no need for a hand-drawn overlay here. */}

        {/* Regional — catchment outline, with the 3-day-out forecast advisory zone over it */}
        {detailLevel === "regional" && (
          <>
            <Polygon positions={NEWRY_CATCHMENT.boundary}
              pathOptions={{ color: T.n1000, weight: 1.5, fillColor: T.n1000, fillOpacity: 0.03, dashArray: "4 5" }} />
            <Circle center={REGIONAL_FORECAST.center} radius={REGIONAL_FORECAST.radiusM}
              pathOptions={{ color: T.orange600, weight: 2, fillColor: T.orange600, fillOpacity: 0.18, dashArray: "6 4" }} />
          </>
        )}

        {/* Level 2 — approximate flood-extent polygons for the town centre */}
        {detailLevel === "l2" && (
          <>
            <Polygon positions={FLOOD_EXTENT.modelled}
              pathOptions={{ color: "#9B5DE5", weight: 1.5, fillColor: "#9B5DE5", fillOpacity: 0.14, dashArray: "3 3" }} />
            <Polygon positions={FLOOD_EXTENT.observed}
              pathOptions={{ color: "#00BBD3", weight: 2, fillColor: "#00BBD3", fillOpacity: 0.28 }} />
          </>
        )}

        {/* Level 3 — street-level risk markers, colour-coded */}
        {detailLevel === "l3" && STREET_RISK.map((s) => (
          <CircleMarker key={s.name} center={[s.lat, s.lng]} radius={8}
            pathOptions={{ color: T.white, fillColor: STREET_RISK_COLOR[s.risk], fillOpacity: 0.95, weight: 2 }}>
            <Popup><div style={{ fontSize: 12, maxWidth: 220 }}><strong>{s.name}</strong><br/>{s.note}</div></Popup>
          </CircleMarker>
        ))}
...
function NewryMapSchematic({ assets, mode, params, selectedId, onSelect, showScheme }) {
  return (
    <div className="absolute inset-0" style={{ background: T.land }}>
      <svg viewBox="0 0 1000 640" className="w-full h-full" preserveAspectRatio="xMidYMid slice" role="img"
        aria-label="Newry and flood alleviation scheme schematic">
        {/* --- base land --- */}
        ...
        {/* --- water: Newry River corridor (west) --- */}
        ...
        {/* Camlough River confluence (secondary) */}
        <rect x="46" y="392" width="86" height="120" rx="8" fill={T.water} stroke={T.waterDeep} strokeWidth="1.5" />
        <text x="52" y="384" fontSize="10" fill={T.n1100} className="fiq">Camlough R.</text>

        {/* Newry Canal across the town */}
        ...
        <text x="600" y="336" fontSize="10" fill={T.n1100} className="fiq">Newry Canal</text>
        ...
        <g>
          ...
          <text x="252" y="410" fontSize="11" fontWeight="600" fill={T.n1} className="fiq">Newry Flood Alleviation Scheme</text>
        </g>
        ...
        <text x="430" y="200" fontSize="10" fontWeight="600" fill={T.n1} className="fiq">Newry City</text>
        ...
        {/* --- settlements --- */}
        {[["Bessbrook",176,104],["Camlough",236,110],["Poyntzpass",264,86],["Meigh",420,104],
          ["Rostrevor",762,344],["Warrenpoint",512,412],["Newry",188,566],["Rathfriland",700,470]]
          .map(([n,x,y])=>(
...
function SideNav({ mode, setMode, panels, togglePanel, expanded, setExpanded, page, setPage, viewMode }) {
  const navItems = [
    { k: "live", icon: "mode1", label: "Eyes on the system", type: "mode" },
    { k: "incidents", icon: "mode2", label: "Incident management", type: "mode", internalOnly: true },
    { k: "emergency", icon: "mode3", label: "Emergency management", type: "page", internalOnly: true },
    { k: "comms_page", icon: "mode4", label: "Communications", type: "page", internalOnly: true },
    { k: "event2023", icon: "mode5", label: "2023 Flood Event", type: "page" },
  ].filter((item) => viewMode === "internal" || !item.internalOnly);
...
function TopNav({ published, commsOpen, setCommsOpen, unreadFeeds, storm, onToggleStorm,
                 weather, simProgress, setSimProgress, canScrub, viewMode, setViewMode }) {
  return (
    <header className="fiq flex items-center shrink-0"
      ...
      <div className="flex items-center min-w-0" style={{ flex: 1, gap: 14 }}>
        ...
        <div className="flex" style={{ gap: 2, background: T.surface1, border: `1px solid ${T.borderPrimary}`, borderRadius: T.r2, padding: 2 }}>
          {[{ k: "internal", label: "Internal" }, { k: "public", label: "Public" }].map((o) => (
            <button key={o.k} onClick={() => setViewMode(o.k)}
              title={o.k === "internal" ? "Full operational detail — assets, incidents, teams" : "Simplified view — what the public sees"}
              style={{ fontSize: 11, fontWeight: 600, padding: "4px 9px", borderRadius: T.r2, border: "none", cursor: "pointer",
                background: viewMode === o.k ? T.blue800 : "transparent", color: viewMode === o.k ? T.white : T.n1 }}>
              {o.label}
            </button>
          ))}
        </div>
...
/* Regional / Level 1 / Level 2 / Level 3 view switcher, per the client's
   ask for different levels of detail for different audiences (public vs
   internal, gauge data vs area impact vs street-level). */
function DetailLevelControl({ detailLevel, setDetailLevel, viewMode }) {
  const options = [
    { k: "regional", label: "Regional" },
    { k: "l1", label: "Level 1 — Gauges", internalOnly: true },
    { k: "l2", label: "Level 2 — Area" },
    { k: "l3", label: "Level 3 — Streets" },
  ].filter((o) => viewMode === "internal" || !o.internalOnly);
  return (
    <div className="fiq absolute flex" style={{
      left: "50%", top: 8, transform: "translateX(-50%)", zIndex: 500, gap: 2,
      background: T.surface1, border: `1px solid ${T.borderPrimary}`, borderRadius: T.r4, padding: 3, boxShadow: T.shadow,
    }}>
      {options.map((o) => (
        <button key={o.k} onClick={() => setDetailLevel(o.k)}
          style={{
            fontSize: 11, fontWeight: 600, padding: "5px 10px", borderRadius: T.r2, cursor: "pointer", border: "none",
            whiteSpace: "nowrap",
            background: detailLevel === o.k ? T.blue800 : "transparent",
            color: detailLevel === o.k ? T.white : T.n1,
          }}>
          {o.label}
        </button>
      ))}
...
/* Level 3 companion list — the street-by-street risk detail, using the
   real streets named in the DfI review as flooded / at-risk in Oct 2023. */
function StreetRiskPanel() {
  const order = { high: 0, medium: 1, low: 2 };
  const rows = [...STREET_RISK].sort((a, b) => order[a.risk] - order[b.risk]);
  return (
    <div className="fiq absolute flex flex-col" style={{
      right: 8, top: 48, width: 280, maxHeight: "calc(100% - 120px)", zIndex: 500,
      background: T.surface1, border: `1px solid ${T.borderPrimary}`, borderRadius: T.r4, boxShadow: T.shadow, overflow: "hidden",
    }}>
      <div style={{ padding: "10px 12px", borderBottom: `1px solid ${T.borderPrimary}`, fontSize: 13, fontWeight: 600, color: T.n1 }}>
        Street-level risk
      </div>
      <div style={{ overflowY: "auto", padding: "4px 0" }}>
        {rows.map((s) => (
          <div key={s.name} style={{ padding: "7px 12px", borderBottom: `1px solid ${T.n500}` }}>
            <div className="flex items-center" style={{ gap: 7 }}>
              <span style={{ width: 8, height: 8, borderRadius: 999, background: STREET_RISK_COLOR[s.risk], flexShrink: 0 }} />
              <span style={{ fontSize: 12, fontWeight: 600, color: T.n1 }}>{s.name}</span>
            </div>
            <div style={{ fontSize: 11, color: T.n1000, marginTop: 2, lineHeight: 1.35 }}>{s.note}</div>
          </div>
        ))}
      </div>
    </div>
...
/* Regional companion card — names the catchment and the forecast horizon
   shown by the amber blob, so the map isn't just an unlabelled overlay. */
function RegionalInfoPanel() {
  return (
    <div className="fiq absolute flex flex-col" style={{
      right: 8, top: 48, width: 260, zIndex: 500,
      background: T.surface1, border: `1px solid ${T.borderPrimary}`, borderRadius: T.r4, boxShadow: T.shadow, padding: "10px 12px",
    }}>
      <div style={{ fontSize: 13, fontWeight: 600, color: T.n1, marginBottom: 6 }}>Newry River catchment</div>
      <div className="flex items-center" style={{ gap: 7, marginBottom: 6 }}>
        <span style={{ width: 10, height: 10, borderRadius: 999, background: T.orange600, flexShrink: 0 }} />
        <span style={{ fontSize: 12, color: T.n1 }}>Amber — elevated river levels possible</span>
      </div>
      <div style={{ fontSize: 11, color: T.n1000, lineHeight: 1.4 }}>
        Forecast horizon: 3 days out. Covers the Newry, Camlough and Bessbrook catchment draining to Carlingford Lough.
      </div>
    </div>
  );
}
...
/* Detail for one NI flood warning area, opened from its map triangle into
   the bottom dock's Detail tab. */
    ...
    <div>
      ...
      <div style={{ padding: 12, maxWidth: 460 }}>
        <div className="flex items-start" style={{ gap: 8 }}>
          <NiMark tier={flood.tier} size={30} />
        ...
        {flood.demo && (
          <div style={{ fontSize: 10, color: T.orange1200, background: T.orange100, borderRadius: T.r2,
            ...
            Demonstration severity on a real Newry-area watercourse. No live DfI Rivers warning feed is integrated yet.
          </div>
        )}
      </div>
    </div>
...
function NetworkDock({
  ...
  const filteredEa = NI_WARNINGS.filter((w) => {
  ...
  return (
    <section className="fiq flex flex-col shrink-0"
      ...
      <header className="flex items-center shrink-0" style={{ height: 36, background: T.n400, borderBottom: `1px solid ${T.borderPrimary}` }}>
        ...
        {tabBtn("ea", "Flood Warnings", filteredEa.length)}
...
function ScenarioPanel({ params, setParams, assets, onClose }) {
  ...
  return (
    <Panel title="Scenario — canal operation" onClose={onClose}
      ...
      <div style={{ padding: 10 }}>
        ...
        <S label="Starting canal pound fill" k="startFull" min={55} max={100} unit="%" />
        <S label="Pre-emptive drawdown" k="drawdown" min={0} max={100} unit="%"
          help="Buys freeboard — at the cost of Newry flow downstream." />
        ...
        {[
          ...
          { label: "Newry River at Victoria Lock", v: `${colneProj.toFixed(2)} m`, st: colneSt, note: "Tidal — Newry / Camlough receptors" },
        ...
        <div style={{ marginTop: 10, padding: 8, borderRadius: T.r4, background: T.orange100, border: `1px solid ${T.orange500}` }}>
          <div style={{ fontSize: 11, color: T.orange1200, lineHeight: 1.45 }}>
            ...
            released raises the Newry through Camlough and Newry. Two populations, opposite directions.
          </div>
        </div>

        <div style={{ fontSize: 10, color: T.n1000, marginTop: 8, lineHeight: 1.45 }}>
          Simplified response surface for prototyping. Swap for Flood Modeller / canal routing outputs via API.
        </div>
      </div>
    </Panel>
  );
...
const TIMELINE_TAG_META = {
  weather: { label: "Weather", color: T.blue800 },
  action: { label: "Response action", color: T.n1000 },
  incident: { label: "Incident", color: T.red800 },
  gauge: { label: "Gauge", color: T.orange600 },
  note: { label: "Note", color: T.n1000 },
};

function FloodEventTimeline({ onLoadEvent, loaded }) {
  return (
    <div className="flex flex-col h-full" style={{ overflowY: "auto", background: T.n300 }}>
      <div style={{ padding: "24px 32px", maxWidth: 860, margin: "0 auto", width: "100%" }}>
        <div className="flex items-center justify-between" style={{ marginBottom: 4, gap: 12, flexWrap: "wrap" }}>
          <div>
            <div style={{ fontSize: 20, fontWeight: 700, color: T.n1 }}>2023 Flood Event — Newry</div>
            <div style={{ fontSize: 12, color: T.n1000, marginTop: 2 }}>
              Storm Babet &amp; the 26 Oct–1 Nov 2023 rainfall event. Source: DfI Rivers, "Review of Flooding in the
              South East, Autumn 2023" (Aug 2024).
            </div>
          </div>
          <button onClick={onLoadEvent}
            style={{
              fontSize: 12, fontWeight: 600, padding: "8px 14px", borderRadius: T.r2, cursor: "pointer",
              background: loaded ? T.n500 : T.blue800, color: loaded ? T.n1000 : T.white, border: "none",
              whiteSpace: "nowrap",
            }}>
            {loaded ? "2023 event loaded ✓" : "Load 2023 event into incidents"}
          </button>
        </div>

        <div style={{ marginTop: 20, position: "relative", paddingLeft: 20 }}>
          <div style={{ position: "absolute", left: 5, top: 6, bottom: 6, width: 2, background: T.n600 }} />
          {FLOOD_2023_TIMELINE.map((ev, i) => {
            const meta = TIMELINE_TAG_META[ev.tag] || TIMELINE_TAG_META.note;
            const dotColor = ev.severity === "severe" ? T.red800 : ev.severity === "warning" ? T.orange600 : meta.color;
            return (
              <div key={i} className="flex" style={{ gap: 14, position: "relative", paddingBottom: 18 }}>
                <div style={{
                  position: "absolute", left: -20, top: 3, width: 12, height: 12, borderRadius: 999,
                  background: dotColor, border: `2px solid ${T.n300}`,
                }} />
                <div style={{ minWidth: 120, paddingTop: 0 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: T.n1 }}>{ev.date}</div>
                  {ev.time && <div style={{ fontSize: 11, color: T.n1000 }}>{ev.time}</div>}
                </div>
                <div className="flex-1" style={{ background: T.surface1, border: `1px solid ${T.n600}`, borderRadius: T.r2, padding: "10px 12px" }}>
                  <div className="flex items-center" style={{ gap: 8, marginBottom: 2 }}>
                    <span style={{ fontSize: 13, fontWeight: 600, color: T.n1 }}>{ev.title}</span>
                    <span style={{
                      fontSize: 10, fontWeight: 600, padding: "1px 6px", borderRadius: 999,
                      background: meta.color, color: T.white, opacity: 0.85,
                    }}>{meta.label}</span>
                  </div>
                  <div style={{ fontSize: 12, color: T.n1000, lineHeight: 1.4 }}>{ev.detail}</div>
                </div>
              </div>
            );
          })}
        ...
        <div style={{ marginTop: 8, marginBottom: 24, fontSize: 11, color: T.n1000 }}>
          Figures based on Section 2.2 (Figure 2-2) and Table 3-13 of the DfI review. Rebuilt as original UI —
          not a reproduction of the source report's graphics.
        </div>
      </div>
    </div>
  );
...
function IncidentsBody({ incidents, setIncidents, selectedIncId, setSelectedIncId, onDraftAlert, onSelect, onZoomTo }) {
  ...
  const notifyServices = (id) => {
    ...
    addLog(id, "Emergency services notified — NIFRS + PSNI acknowledged");
  };
  const deploySandbags = (id) => {
    setIncidents((c) => c.map((i) => (i.id === id ? { ...i, sandbagsDeployed: true } : i)));
    addLog(id, "Sandbag deployment actioned — NMD Council stores dispatched (ref. DfI recommendation A10)");
  };
  const [teamMenuOpen, setTeamMenuOpen] = useState(false);
  const assignTeam = (id, team) => {
    setIncidents((c) => c.map((i) => (i.id === id ? { ...i, assignedTeam: team.id } : i)));
    addLog(id, `${team.name} assigned (${team.org})`);
    ...
    <div>
      {open.length === 0 && (
        <div style={{ padding: 12, fontSize: 12, color: T.n1000, lineHeight: 1.5 }}>
          ...
          and wait for the canal pound to climb.
        </div>
      )}
          ...
          {inc && (
            <div style={{ padding: 10 }}>
              <div className="flex items-start justify-between" style={{ gap: 8 }}>
                <div className="min-w-0">
                  ...
                  <div className="fiq-mono" style={{ fontSize: 10, color: T.n1000 }}>
                    opened {inc.opened} · {inc.status}{inc.servicesNotified ? " · services ✓" : ""}{inc.sandbagsDeployed ? " · sandbags ✓" : ""}
                  </div>
                </div>
              ...
              <div className="flex" style={{ gap: 5, marginTop: 9, flexWrap: "wrap" }}>
                ...
                <button onClick={() => !inc.sandbagsDeployed && deploySandbags(inc.id)} disabled={inc.sandbagsDeployed}
                  style={{ fontSize: 11, fontWeight: 600, padding: "4px 9px", borderRadius: T.r2,
                    background: inc.sandbagsDeployed ? T.n400 : T.orange600, color: inc.sandbagsDeployed ? T.n1000 : T.white }}>
                  {inc.sandbagsDeployed ? "✓ Sandbags deployed" : "Deploy sandbags"}
                </button>
                <div style={{ position: "relative" }}>
                  <button onClick={() => setTeamMenuOpen((v) => !v)}
                    ...
                    {inc.assignedTeam ? `✓ ${DUTY_TEAMS.find((t) => t.id === inc.assignedTeam)?.name || "Team assigned"}` : "Assign team"}
                  </button>
                  {teamMenuOpen && (
                    <div style={{ position: "absolute", left: 0, top: "100%", marginTop: 4, background: T.surface1,
                      border: `1px solid ${T.borderPrimary}`, borderRadius: T.r3, padding: 4, boxShadow: T.shadow, zIndex: 600, minWidth: 220 }}>
                      {DUTY_TEAMS.map((team) => (
                        <button key={team.id} onClick={() => assignTeam(inc.id, team)}
                          style={{ display: "block", width: "100%", textAlign: "left", padding: "6px 8px", borderRadius: T.r2,
                            fontSize: 11, color: T.n1, background: "transparent", border: "none", cursor: "pointer" }}>
                          <div style={{ fontWeight: 600 }}>{team.name}</div>
                          <div style={{ color: T.n1000, fontSize: 10 }}>{team.org} · {team.shift} · {team.members.join(", ")}</div>
                        </button>
                      ))}
                    </div>
                  )}
...
function CommsDrawer({
  ...
  const draft = () => {
    ...
    if (audience === "ops") {
      setMessage(`NMD OPS — ${LEVELS[st].label.toUpperCase()}. ${w.name} at ${fmt(w.level, w.unit)}${w.unit}. Expect town-centre road flooding and possible closures. DfI Rivers duty to brief street cleansing. Drainage team to Edward Street balancing pond.`);
    } else if (audience === "emerg") {
      setMessage(`MULTI-AGENCY — ${LEVELS[st].label.toUpperCase()} declared, Newry Flood Alleviation Scheme. ${w.name} at ${fmt(w.level, w.unit)}${w.unit}. Scheme operating authority (DfI Rivers) on site. Requesting standby at Sugar Island and Bessbrook. Off-site plan reference held by NIFRS.`);
    } else {
      setMessage(`FLOOD ${st === "severe" ? "DANGER" : "WARNING"} — Newry, Camlough and Bessbrook. River levels are rising and flooding of roads and some homes is likely tonight. Do not walk or drive through flood water. Move your car and anything valuable upstairs or to higher ground now. Updates: DfI Rivers Floodline 0300 200 7837.`);
    }
  };
  ...
  const simplify = () => {
    ...
    setTimeout(() => {
      ...
      const unit = w.unit;
      const area = w.group === "res" ? "canal" : w.group === "river" ? "river" : "watercourse";
      const locations = ["Newry", "Camlough", "Bessbrook", "Warrenpoint"];
      ...
      if (audience === "public") {
        ...
        rewritten = `FLOOD ${st === "severe" ? "DANGER" : "WARNING"} — ${locStr}. ${urgency} Water levels at ${w.name} are ${level}${unit}. ${action} For help call DfI Rivers Floodline 0300 200 7837.`;
      } else if (audience === "ops") {
        rewritten = `NMD OPS — ${LEVELS[st].label.toUpperCase()}: ${w.name} ${level}${unit}. ${st === "severe" ? "Expect town-centre road flooding and possible closures. DfI Rivers duty to brief street cleansing. Drainage team to Edward Street balancing pond." : "Monitor town drainage. Brief street cleansing if levels rise further."}`;
      } else {
        rewritten = `MULTI-AGENCY — ${LEVELS[st].label.toUpperCase()}, Newry ${area} scheme. ${w.name} ${level}${unit}. ${st === "severe" ? "Scheme operating authority on site. Requesting standby at Sugar Island and Bessbrook. Off-site plan ref held by NIFRS." : "Scheme monitoring. Standing by for escalation. Update channel: ops-bridge."}`;
      }
...
export default function FloodIqNewry() {
  ...
  const [event2023Loaded, setEvent2023Loaded] = useState(false);
  ...
  const [detailLevel, setDetailLevel] = useState("l1");
  ...
  const [cursorPos, setCursorPos] = useState({ lat: NEWRY_VIEW.center[0], lng: NEWRY_VIEW.center[1] });
  ...
  /* NI flood warnings — demonstration data (see niFloods.js for why there's
     no live DfI Rivers feed yet). Re-checked every 15 minutes to mirror the
     polling cadence a real integration would use. */
  useEffect(() => {
    const ac = new AbortController();
    const load = () =>
      fetchNiFloods({ centre: NEWRY_VIEW.center, signal: ac.signal })
    ...
    const id = setInterval(() => {
      setAssets((cur) => {
        ...
        return cur.map((a) => {
          ...
          let drift;
          if (a.id === "res-fb") {
            // freeboard is derived from canal pound level
  ...
  /* Public view has no incident/emergency/comms management surfaces or
     raw gauge telemetry (Level 1) — fall back to safe defaults when
     switching into it so nothing hidden stays "stuck" open. */
  useEffect(() => {
    if (viewMode !== "public") return;
    if (mode === "incidents") setMode("live");
    if (page === "emergency" || page === "comms_page") setPage(null);
    if (detailLevel === "l1") setDetailLevel("l2");
  }, [viewMode]);
  ...
  const draftFromIncident = (inc) => {
    ...
    setMessage(`FLOOD ${inc.severity === "severe" ? "DANGER" : "WARNING"} — Newry, Camlough and Bessbrook. ${names} has reached ${LEVELS[inc.severity].label}. Flooding of roads and some homes is likely. Do not walk or drive through flood water. Move your car and valuables to higher ground now. Updates: DfI Rivers Floodline 0300 200 7837.`);
    setCommsTab("notify");
    setCommsOpen(true);
  };

  const loadFlood2023Event = () => {
    if (event2023Loaded) return;
    const seeded = FLOOD_2023_INCIDENTS.map((tpl) => ({
      id: nextInc.current++,
      title: `${tpl.ref} — ${tpl.title}`,
      severity: tpl.severity,
      assetIds: tpl.assetIds,
      status: "monitoring",
      servicesNotified: true,
      sandbagsDeployed: true,
      opened: tpl.opened,
      log: tpl.log,
    }));
    setIncidents((c) => [...seeded, ...c]);
    setSelectedIncId(seeded[0].id);
    setEvent2023Loaded(true);
    setDockTab("incidents");
    setDockOpen(true);
  };
  ...
  return (
    <div className="fiq flex" style={{ height: "100vh", background: T.surface1, color: T.n1 }}>
      ...
      <SideNav mode={mode} setMode={setMode} panels={{ ...panels, network: dockOpen && dockTab === "network",
        ...
        page={page} setPage={setPage} viewMode={viewMode} />
      ...
      <div className="flex flex-col flex-1 min-w-0">
        <TopNav published={published}
          ...
          canScrub={!!forecast} viewMode={viewMode} setViewMode={setViewMode} />

        <div className="relative flex-1 min-h-0" style={{ overflow: "hidden" }}>
          {page ? (
            page === "event2023" ? (
              <FloodEventTimeline onLoadEvent={loadFlood2023Event} loaded={event2023Loaded} />
            ) : (
            <div className="flex flex-col items-center justify-center h-full" style={{ background: T.n400, gap: 12 }}>
              <FigIcon name={page === "emergency" ? "mode3" : "mode4"} size={32} />
              <span style={{ fontSize: 16, fontWeight: 600, color: T.n1 }}>
                {page === "emergency" ? "Emergency management" : "Communications"}
              </span>
              <span style={{ fontSize: 12, color: T.n1000 }}>Coming soon</span>
            </div>
            )
          ) : (<>
          <NewryMap assets={viewAssets} mode={mode} params={params}
          ...
          <DetailLevelControl detailLevel={detailLevel} setDetailLevel={setDetailLevel} viewMode={viewMode} />
          {detailLevel === "l3" && <StreetRiskPanel />}
          {detailLevel === "regional" && <RegionalInfoPanel />}
